...
export const getAllProjectsInternal = async (req, res) => {
  try {
    const projects = await Project.find({})
      .select('_id name end_date status owner_id team_id')
      .lean();
    
    res.json(projects);
  } catch (error) {
    console.error('❌ Lỗi getAllProjectsInternal:', error.message);
    res.status(500).json({ message: 'Lỗi server', error: error.message });
  }
};